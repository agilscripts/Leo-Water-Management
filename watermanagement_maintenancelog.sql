...
CREATE TABLE `maintenancelog` (
  ...
  `maintenance_type` enum('Scheduled','Unscheduled','Emergency') DEFAULT NULL,
  `description` varchar(100) DEFAULT NULL,
  `user_id` int DEFAULT NULL,
  `sensor_id` int DEFAULT NULL,
  `status` enum('Completed','InProgress','Pending') DEFAULT NULL,
  PRIMARY KEY (`maintenance_id`),
  KEY `user_id` (`user_id`),
  KEY `sensor_id` (`sensor_id`),
  CONSTRAINT `maintenancelog_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `user` (`user_id`),
  CONSTRAINT `maintenancelog_ibfk_2` FOREIGN KEY (`sensor_id`) REFERENCES `sensordata` (`sensor_id`)
